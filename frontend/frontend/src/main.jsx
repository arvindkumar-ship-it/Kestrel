...
function Alerts() {
  ...
  async function revokeActor(actor) {
    try {
      const q = new URLSearchParams({ session_id: actor, reason: "Anomaly detected", actor: "security-admin" }).toString();
      const r = await api(`/response/revoke-session?${q}`, { method: "POST", body: "{}" });
      setRespMsg(r.applied ? `Access revoked for ${actor}` : r.reason);
    } catch (e) { setRespMsg(e.message); }
  }
  async function testAccess(actor) {
    try {
      await api("/dashboard/alerts", { headers: { "X-Session-ID": actor } });
      setRespMsg(`200 OK - ${actor} still has access`);
    } catch (e) {
      setRespMsg(e.message === "session revoked" ? `401 - ${actor}: session revoked, access blocked` : e.message);
    }
  }
      ...
      {selected && (
        <div className="threat-panel">
          ...
          <div className="explain">
            <span>RESPONSE</span>
            <p>
              <button onClick={() => revokeActor(selected.detail?.actor_id)}>Revoke access</button>{" "}
              <button onClick={() => testAccess(selected.detail?.actor_id)}>Test access</button>
              {respMsg && <b> {respMsg}</b>}
            </p>
          </div>
          <div className="explain">
